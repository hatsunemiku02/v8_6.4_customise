...
typename ParserBase<Impl>::BlockT ParserBase<Impl>::ParseBlock(
  ...
  Expect(Token::LBRACE, CHECK_OK_CUSTOM(NullStatement));
  {
    ...
    while (peek() != Token::RBRACE) {
      /*
      if (V8_PluginCFG::GetInstance().V8_InjectFoo)
      {
        const char* fooname = "CallBackTest";
        const Runtime::Function* function = Runtime::FunctionForName((const unsigned char*)fooname, 12);
        ZoneList<Expression*>* arguments = new (zone()) ZoneList<Expression*>(0, zone());
        body->statements()->Add(factory()->NewExpressionStatement(factory()->NewCallRuntime(function, arguments, peek_position()), peek_position()), zone());
      }
      */
