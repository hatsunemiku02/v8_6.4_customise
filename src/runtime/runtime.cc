...
//static const Runtime::Function native_kIntrinsicFunctions[] = {
//
//};
/*
static int i = 0;
Object* CallBackTest(Arguments& args, Isolate* isolate)
{
...
std::vector<Runtime::Function> kIntrinsicFunctions
{
...
std::map<int, std::string> UserAddedFooMap;

void AddIntrinsicFoo( const char* name, void* fooaddr, int paramsize, int resultsize)
{
  int idx = kIntrinsicFunctions.size();
  Runtime::Function foo;
  foo.entry = reinterpret_cast<v8::internal::Address>(fooaddr);
  foo.function_id = (Runtime::FunctionId)idx;
  foo.name = name;
  foo.intrinsic_type = Runtime::INLINE;
  foo.nargs = paramsize;
  foo.result_size = resultsize;
  kIntrinsicFunctions.push_back(foo);
  UserAddedFooMap[idx] = name;
}

const char* GetUserAddedFooNameFromIdx(int index)
{
  return UserAddedFooMap[index].c_str();
}
...
void InitializeIntrinsicFunctionNames() {
  ...
  //int arrsize = arraysize(kIntrinsicFunctions);
  int arrsize = kIntrinsicFunctions.size();
...
const Runtime::Function* Runtime::FunctionForEntry(Address entry) {
 // for (size_t i = 0; i < arraysize(kIntrinsicFunctions); ++i) {
  for (size_t i = 0; i <kIntrinsicFunctions.size(); ++i) {
...
const Runtime::Function* Runtime::RuntimeFunctionTable(Isolate* isolate) {
  if (isolate->external_reference_redirector()) {
   ...
   //   size_t function_count = arraysize(kIntrinsicFunctions);
      size_t function_count = kIntrinsicFunctions.size();
   ...
   //   memcpy(redirected_functions, kIntrinsicFunctions,sizeof(kIntrinsicFunctions));
      memcpy(redirected_functions, &kIntrinsicFunctions[0], sizeof(Function)*kIntrinsicFunctions.size());
