...
namespace interpreter {
  ...
  class ARBRegisterAllocationScope final {
  public:
    explicit ARBRegisterAllocationScope(BytecodeArrayBuilder* generator)
      : generator_(generator),
      outer_next_register_index_(
        generator_->register_allocator()->next_register_index()) {}

    ~ARBRegisterAllocationScope() {
      generator_->register_allocator()->ReleaseRegisters(
        outer_next_register_index_);
    }

  private:
    BytecodeArrayBuilder * generator_;
    int outer_next_register_index_;

    DISALLOW_COPY_AND_ASSIGN(ARBRegisterAllocationScope);
  };
...
void BytecodeArrayBuilder::ExtraWrite(BytecodeNode* node) {
  AttachOrEmitDeferredSourceInfo(node);
  bytecode_array_writer_.ExteraWrite(node);
}

void BytecodeArrayBuilder::Write(BytecodeNode* node) {
  AttachOrEmitDeferredSourceInfo(node);
  bytecode_array_writer_.Write(node);
  if (V8_PluginCFG::GetInstance().V8_InjectFoo==true)
  {
     V8_PluginCFG::GetInstance().OuputCount++;
     V8_PluginCFG::GetInstance().V8_InjectFoo = false;
     ARBRegisterAllocationScope register_scope();
     RegisterList args = register_allocator()->NewRegisterList(2);

     this
       ->StoreAccumulatorInRegister(args[0])
       .ExtraLoadNumber((int)node->bytecode())
       .StoreAccumulatorInRegister(args[1])
       .ExtraCallRuntime(Runtime::KUserAddedFoo, args);
...
#undef DEFINE_BYTECODE_OUTPUT
  ...
  template <typename... Operands>                                     
  void BytecodeArrayBuilder::ExtraOutputCallRuntime(Operands... operands) {
  
    BytecodeNode node(CreateCallRuntimeNode(operands...));
    ExtraWrite(&node);                                                     
  }                                                                   
                                                                      
  template <typename... Operands>                                     
  void BytecodeArrayBuilder::ExtraOutputCallRuntime(BytecodeLabel* label,
                                          Operands... operands) {     
    DCHECK(Bytecodes::IsJump(Bytecode::kCallRuntime));
    BytecodeNode node(CreateCallRuntimeNode(operands...));
    ExtraWriteJump(&node, label);
    LeaveBasicBlock();                                                
  }

  template <typename... Operands>
  void BytecodeArrayBuilder::ExtraOutputInvokeIntrinsic(Operands... operands) {

    BytecodeNode node(CreateInvokeIntrinsicNode(operands...));
    ExtraWrite(&node);
  }

  template <typename... Operands>
  void BytecodeArrayBuilder::ExtraOutputInvokeIntrinsic(BytecodeLabel* label,
    Operands... operands) {
    DCHECK(Bytecodes::IsJump(Bytecode::InvokeIntrinsic));
    BytecodeNode node(CreateInvokeIntrinsicNode(operands...));
    ExtraWriteJump(&node, label);
    LeaveBasicBlock();
  }
...
BytecodeArrayBuilder& BytecodeArrayBuilder::ExtraLoadNumber(int32_t raw_smi)
{
  if (raw_smi == 0) {
    OutputLdaZero();
  }
...
BytecodeArrayBuilder& BytecodeArrayBuilder::ExtraCallRuntime(
  ...
  if (IntrinsicsHelper::IsSupported(function_id)) {
    ...
    ExtraOutputInvokeIntrinsic(static_cast<int>(intrinsic_id), args,
      args.register_count());
  }
  else {
    ExtraOutputCallRuntime(static_cast<int>(function_id), args,
      args.register_count());
  }
  return *this;
}
